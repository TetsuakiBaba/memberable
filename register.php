...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (
        ...
        try {
            ...
            // アカウント作成に成功 member_idの更新
            $stmt = $db->prepare("UPDATE users SET member_id = ? WHERE id = ?");
            $stmt->execute([$member_id, $user_id]);
            echo '<script> document.addEventListener("DOMContentLoaded", async () => {
             goToLogin();
        });</script>';
        } catch (Exception $e) {
...
<body>
    <div class="container-sm">
        <div class="row mb-3">
            ...
            <div id="filling_item" class="alert alert-warning alert-dismissible fade show mt-3" role="alert">
            ...
            <?php endif; ?>
            <div id="filling_items">
                <form method="post">
                    <!-- Email -->
                    <div id="filling_item" class="form-group mb-3">
                    ...
                    <div id="filling_item" class="form-check mb-3">
                        <input type="checkbox" class="form-check-input" name="agree" value="agree" required>
                        <label class="form-check-label" for="flexCheckDefault">
                            <?php echo AGREE_TEXT; ?>
                        </label>
                    </div>
                    <button id="filling_item" type="submit" class="btn btn-primary">
                        Register
                    </button>
            </div>
            </form>
            <p class="mt-3"><span id="last_message"> Already have an account?</span> <a href="login.php">Login here</a>.</p>
        </div>
    ...
    <script>
        // Select all elements with id="filling_item"
        var elements = document.querySelectorAll("#filling_item");
        ...
        // Function to fade out an element over 0.5 seconds
        function fadeOutElement(el) {
            return new Promise(function(resolve) {
                ...
                // Trigger reflow to make sure the transition is applied
                void el.offsetWidth;
                // Set opacity to 0 to start the fade-out
                el.style.opacity = '0';
        ...
        async function goToLogin() {
            // Apply the fade-out effect sequentially
            await elementsArray.reduce(function(promise, element) {
                return promise.then(function() {
                    return fadeOutElement(element);
                });
            }, Promise.resolve());
            const alert = document.getElementById('last_message');
            // Function to write 'Success' to the element's text every 0.1 seconds
            let message = "Registration is Successful > ";
            let index = 0;
            alert.innerHTML = '';
            alert.classList.add('bg-success', 'text-white', 'p-1', 'rounded');

            function typeMessage() {
                if (index < message.length) {
                    alert.innerHTML += message.charAt(index);
                    index++;
